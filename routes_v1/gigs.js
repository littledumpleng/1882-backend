...
router.get("/", async (req, res) => {
  try {
    console.log("Touched /Gigs with GET");
    // since we already pointed '/gigs' to this file, we just need to use '/'
    ...
    const gigs = await Gig.findAll(queryParams); // findAll() is a sequelize method to find gigs
    console.log("AWAIT COMPLETE");
    res.json({ data: gigs }); // sends gigs as json
  } catch (err) {
    console.log(err);
    res.send(err);
  }
